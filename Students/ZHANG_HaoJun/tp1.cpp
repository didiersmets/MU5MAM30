...
struct SparseMatrix {
	int rows;
	int cols;
	int nnz; // number of non zero elems
	struct Coeff *coeffs;
};
...
{
	struct vector V =  {A.y*B.z - A.z*B.y,
						A.z*B.x - A.x*B.z,
						A.x*B.y - A.y*B.x};
	return V;
}
...
void build_fem_matrices(const struct Mesh *m, struct SparseMatrix *S, struct SparseMatrix *M)
{
	...
	for (int i = 0; i < m->tri_count; i++) {
		...
		double area = 0.5 * norm(cross(CA, AB));

		struct Coeff *mass = &M->coeffs[9 * i];
		mass[0] = {a, a, area / 6};
		mass[1] = {b, b, area / 6};
		mass[2] = {c, c, area / 6};
		mass[3] = {a, b, area / 12};
 ...
 * wherever needed.
 *****************************************************************************/
int gradient_system_solve(const struct SparseMatrix *S, const struct SparseMatrix *M, const double *B, double *U, int N)
	{
		...
		double tol = 1e-6;
		double tol2 = tol * tol;
		while (error2 > tol2 && iter++ < itermax)
		{
			// calculate Ar
			matrix_vector_product(S, U, Ar);
			matrix_vector_product(M, U, Mr);
