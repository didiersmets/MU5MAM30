...
struct SparseMatrix {
	int rows;
	int cols;
	int nnz; // number or non zero coefficients
	struct Coeff *coeffs; // pointer to the first non zero coefficient
};
...
void build_fem_matrices(const struct Mesh *m, struct SparseMatrix *S,
    ...
    for(int t = 0; t < m->tri_count; t++)  {
        ...
        // Maxx coefficients for the triangle
...
int gradient_system_solve(const struct SparseMatrix *S,
    ...
    blas_axpby(1, Mr, 1, r, N); // Mr + r --> r
    blas_axpby(1, B, -1, r, N); // computation of B - Au 

    int iterate = 0;
    int iter_max = 1e3;
    double toll = 1e-6; // tollerance
    double toll2 = toll*toll;
    double err2 = blas_dot(r,r); // error squared

    while(err2 > toll2 && iterate < iter_max) {
        //U_{k+1} = U_{k} + \frac{<r_k, r_k>}{A r_k, r_k} r_k

        // Compute A*r 
        matrix_vector_product(S, r, Ar);
        matrix_vector_product(M, r, Mr);
        blas_axpby(1, Mr, 1, Ar, N);
        double alpha = err2/blas_dot(Ar, r, N);
        // Update U
        blas_axpby(alpha, r, 1, U, N);
        // Update r
        // Note: r_{k+1} = r_k - \alpha_k A r_k
    ...
    free(Mr);
    free(r);
    // TODO: compute the relative error
    *error = sqrt(err2); // user localtion where i will save the error
    return iterate;
}
